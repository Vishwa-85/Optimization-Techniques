%Prg 5: Fibonacci method with comments---
clc
clear all
...
a=((fi(n-1))/fi(n));               %fi(x) is a function to calculate Fn. This term represents: I2/I1 = F(n-1)/F(n) = alpha value
x2=(a*x1)+((1-a)*x4);              
...
while i<n
    ...
    i=i+1;                          %incrementing i for next iteration
     a=((fi(n-i-1))/fi(n-i));       %updating alpha i.e., for next iteration we take (n-i)
    if i==n
        break;
    end      
end
